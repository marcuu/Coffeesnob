...
export default async function VenuesPage({
  ...
  if (selectedRegionData) query = query.in("city", selectedRegionData.cities);
  ...
  const weightedScores =
    venues.length > 0 ? await getVenueOverallScores(supabase, venues.map((v) => v.id)) : new Map();
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        ...
        <form action="/venues" method="get" className="mb-6 flex flex-wrap items-end gap-3">
          <div className="grid flex-1 gap-1.5">
            <label htmlFor="region" className="text-xs font-medium">Filter by region</label>
            <select
              id="region"
              name="region"
            ...
            >
              ...
              {regions.map((r) => (
                <option key={r.id} value={r.id} className="bg-[var(--color-background)] text-[var(--color-foreground)]">
                  {r.name}
                </option>
              ))}
            </select>
          ...
          {selectedRegion ? (
            <Button asChild variant="ghost"><Link href="/venues">Clear</Link></Button>
          ) : null}
        </form>

        {sortedVenues.length === 0 ? (
          <p className="text-sm text-[var(--color-muted-foreground)]">
            {selectedRegion ? `No venues found in "${selectedRegionData?.name ?? selectedRegion}".` : "No venues yet - add the first."}
          </p>
        ) : (
                    ...
                    <Card className="transition-colors hover:bg-[var(--color-muted)]">
                      <CardHeader>
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0">
                            ...
                            <CardDescription>{v.city} · {v.postcode}</CardDescription>
                          </div>
                          <div className="shrink-0 text-right text-sm">
                            <div className="font-medium">{summary.formattedScore}</div>
                            <div className="text-xs text-[var(--color-muted-foreground)]">
                              {summary.rawReviewCount} review{summary.rawReviewCount === 1 ? "" : "s"}
                            </div>
                          </div>
                        </div>
                      ...
                      {v.roasters.length || v.brew_methods.length ? (
                        <CardContent className="flex flex-wrap gap-2 text-xs text-[var(--color-muted-foreground)]">
                          {v.roasters.map((r) => <span key={`r-${r}`} className="rounded-full border border-[var(--color-border)] px-2 py-0.5">{r}</span>)}
                          {v.brew_methods.map((b) => <span key={`b-${b}`} className="rounded-full bg-[var(--color-muted)] px-2 py-0.5">{b.replace("_", " ")}</span>)}
                        </CardContent>
                      ) : null}
                    </Card>
                  </Link>
